...
function doPost(e) {
  try {
    ...
    Logger.log('Parsed data - URL: ' + url + ', Timestamp: ' + timestamp + ', Email: ' + userEmail + ', ID: ' + userId + ', PIN: ' + pin);
    ...
    sheet.appendRow([url, timestamp, userEmail, userId, pin, status]);
    Logger.log('Appended new row with data: ' + [url, timestamp, userEmail, userId, pin, status].join(', '));
