...
function doPost(e) {
  try {
    ...
    // Check if headers exist in the first row; if not, populate them
    var firstRow = sheet.getRange(1, 1, 1, 7).getValues()[0];
    var expectedHeaders = ['URL', 'Title', 'Timestamp', 'Email', 'User ID', 'PIN', 'Status'];
    var headersMissing = firstRow.every(cell => !cell || cell.toString().trim() === '');
    if (headersMissing) {
      Logger.log('Headers not found in first row. Populating headers: ' + expectedHeaders.join(', '));
      sheet.getRange(1, 1, 1, 7).setValues([expectedHeaders]);
    } else {
      Logger.log('Headers already exist in first row: ' + firstRow.join(', '));
    }
